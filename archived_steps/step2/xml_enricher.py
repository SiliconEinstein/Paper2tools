    ...
    for cr in root.findall(".//conclusion_reasoning"):
        ...
        if reasoning is not None:
            for step in reasoning.findall("step"):
                ...
                for tool_id in step_to_tools[step_id]:
                    ...
                    tc = tool_info.get("tool_category")
                    if tc:
                        ref.set("category", str(tc))
        ...
        if tools:
            ...
            for index, tool_info in enumerate(tools, start=1):
                ...
                tid = str(tool_info.get("tool_id") or "").strip()
                if tid:
                    tool_elem.set("tool_id", tid)
                tool_elem.set("index", str(index))
                cat = tool_info.get("tool_category")
                if cat:
                    tool_elem.set("category", str(cat))
