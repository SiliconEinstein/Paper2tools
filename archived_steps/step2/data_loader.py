...
def load_paper_data(paper_id: str, tos_config: dict) -> Optional[PaperData]:
    ...
    try:
        ...
        print(f"[Step2] Failed to load XML for {paper_id}: {e}", flush=True)
        return None
    ...
    if paper_md is None:
        print(f"[Step2] Failed to load MD for {paper_id}", flush=True)
        return None
...
def list_paper_ids(tos_config: dict, limit: Optional[int] = None) -> List[str]:
    ...
    while True:
        ...
        for item in getattr(result, "contents", None) or []:
            ...
            if pid:
                ...
                if n > 0 and n % 5000 == 0:
                    print(
                        f"[Step2] list_objects 进度: 已发现 {n} 个 paper_id "
                        f"（{pages} 页 list，{time.perf_counter() - t0:.1f}s）",
                        flush=True,
                    )
                if limit and len(paper_ids) >= limit:
                    print(
                        f"[Step2] list_paper_ids 达到 sample_limit={limit}，停止列举",
                        flush=True,
                    )
                    return paper_ids
    ...
    print(
        f"[Step2] list_paper_ids 完成: 共 {len(paper_ids)} 个 paper_id，"
        f"{pages} 次 list，耗时 {time.perf_counter() - t0:.1f}s",
        flush=True,
    )
    return paper_ids
