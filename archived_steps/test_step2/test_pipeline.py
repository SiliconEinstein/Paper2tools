...
from src.step2.tool_extractor import _parse_llm_response
...
def test_parse_llm_response_accepts_none():
    """API 偶发 message.content=None 时不应 strip 崩溃。"""
    assert _parse_llm_response(None) == {"tools": [], "links": []}
    assert _parse_llm_response("") == {"tools": [], "links": []}
    out = _parse_llm_response('{"tools": [], "links": []}')
    assert out.get("tools") == [] and out.get("links") == []
